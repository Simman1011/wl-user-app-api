...
const getOrders = asyncHandler(async (req, res) => {
  ...
  try {
    let find = await Order.find({ user: { $in: req.user.id } })
      .limit(limit)
      .skip(skip);
...
const addOrder = asyncHandler(async (req, res) => {
  ...
  let products = await Product.find({ _id: { $in: productIds }, status: "Y" });
  ...
  let orderItems = items.map((item) => {
    ...
    total += subTotal;
    return { product: product._id, color, size, price, qty, subTotal };
  });
  ...
  if (firstOrder && offerCode != "") {
    ...
    if (prevOrder?.length === 0) {
      ...
      return res
        .status(400)
        .json({ error: "This is not the user's first order" });
    }
  }
  ...
  for (const item of items) {
    ...
    if (product) {
      let stockOption = product.stockOptions.find(
        (option) => option.color === item.color
      );
      if (stockOption) {
        let sizeOption = stockOption.options.find(
          (opt) => opt.size === item.size
        );
        if (sizeOption) {
          sizeOption.stocks -= item.qty;
        }
      }
      product.totalStocks -= item.qty;
      await product.save();
    }
  }
...
const getOrderDetails = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (orderDetails !== null) {
      ...
      let partner = await DeliveryPartner.findById(
